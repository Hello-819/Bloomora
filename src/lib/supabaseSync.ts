...
export async function syncAppState(client: SupabaseClient, user: User, local: AppState): Promise<AppState> {
  ...
  await Promise.all([
    client.from(TABLES.profile).upsert(
      {
        id: user.id,
        profile_data: merged.profile,
        gamification_data: merged.gamification,
        updated_at: merged.updatedAt,
      },
      { onConflict: 'id' },
    ),
    client.from(TABLES.labels).upsert(
      merged.labels.map((label) => ({
        user_id: user.id,
        id: label.id,
        name: label.name,
        color: label.color,
    ...
    client.from(TABLES.tasks).upsert(
    ...
    client.from(TABLES.notes).upsert(
    ...
    client.from(TABLES.subjects).upsert(
    ...
    client.from(TABLES.flashcards).upsert(
      merged.flashcards.map((card) => ({
        user_id: user.id,
        id: card.id,
        front: card.front,
        back: card.back,
    ...
    client.from(TABLES.sessions).upsert(
